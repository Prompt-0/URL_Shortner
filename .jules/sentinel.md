...
## 2025-01-01 - Prevent Information Disclosure in API errors
**Vulnerability:** API endpoints were previously catching database errors (e.g. unique constraint violations) and forwarding the unmasked output to the client by including it in the response via `AppError::internal(&format!("database error: {db_err}"))`. This can leak internal schema configurations or environment specifics.
**Learning:** Returning unmasked database output directly to the client is dangerous. To prevent information disclosure, internal database errors must be securely logged server-side and an opaque, generic error message (e.g., "internal server error") should be returned in the HTTP response.
**Prevention:** Avoid returning raw stringified errors (`{e}`) in API payloads. Utilize the tracing crate (`tracing::error!`) to handle logging server-side and return static error variants for internal errors.
