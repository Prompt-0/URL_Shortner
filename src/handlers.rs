...
pub async fn stats(
    ...
    let escaped_short = escape_html(&short_url);
    let escaped_stats = escape_html(&stats_url);
    ...
    let html = render_template(
        ...
        &[
            ("{code}", &escaped_code),
            ("{short_url}", &escaped_short),
            ("{stats_url}", &escaped_stats),
            ("{original_url}", &escaped_original),
