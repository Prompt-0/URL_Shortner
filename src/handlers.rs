...
pub async fn redirect_short_link(
    ...
    if let Some(expires) = &link.expires_at {
        ...
        if link.password.is_some() {
            ...
            return Err(AppError::unauthorized("This link is password protected. UI pending."));
        }
    ...
    // 🛡️ Sentinel: Prevent IP spoofing via X-Forwarded-For
...
pub async fn stats(
    ...
    if link.password.is_some() {
        return Err(AppError::unauthorized("This link is password protected."));
    }
